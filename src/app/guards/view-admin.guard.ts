  ...
  canActivate(route: ActivatedRouteSnapshot,
    ...
    if(!this.usuario.token){
      ...
      if (this.TokenExpired(this.usuario.token)) {
        ...
        this.Token.signOut();
        this.router.navigate(['/signin/master']);
        return false;
      } else {
        //this.expiradaso =  'valido';  
        if( this.usuario.authorities[0].authority == a){
          return true;
        }else{

          if(this.usuario.authorities[0].authority == b || this.usuario.authorities[0].authority == c){
            this.router.navigate(['/error403/master']);     
            return false;
          }else{
            this.router.navigate(['/index']);
            return false;
